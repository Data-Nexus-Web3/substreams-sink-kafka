...
func (ub *UndoBuffer) processBlock(block BufferedBlock) error {
	// If a processing buffer is enabled, enqueue instead of processing synchronously
	if ub.sinker.processingBufferSize > 0 && ub.sinker.processingQueue != nil {
		// Maintain bytes accounting to avoid negative metrics when draining
		var delta int64
		if ub.sinker.processingBufferBytes > 0 {
			delta = sizeEstimate(block.Data)
			atomic.AddInt64(&ub.sinker.processingCurrentBytes, delta)
		}

		select {
		case ub.sinker.processingQueue <- bufferedProcessItem{data: block.Data, isLive: block.IsLive, cursor: block.Cursor}:
			return nil
		default:
			// Revert bytes reservation if we couldn't enqueue
			if delta != 0 {
				atomic.AddInt64(&ub.sinker.processingCurrentBytes, -delta)
			}
			// Fall back to immediate processing if buffer is full
		}
	}
