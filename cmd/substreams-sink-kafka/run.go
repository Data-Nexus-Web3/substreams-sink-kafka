...
import (
	...
	kafkaSinker "github.com/Data-Nexus-Web3/substreams-sink-kafka/kafka/sinker"
...
func addKafkaFlags(flags *pflag.FlagSet) {
	...
	flags.Bool("schema-auto-register", false, "Automatically register schemas if they don't exist")
	flags.Bool("schema-fast-path", false, "Use fast-path SR framing (precompute header; prepend header+payload)")
	...
	// Explosion worker configuration
	flags.Int("explode-workers", 5, "Maximum number of parallel workers for explode processing")
	flags.Bool("explode-always-parallel", true, "Always use parallel workers for explosion, even for small batches")

	// Processing buffer (post-undo) to smooth CPU spikes
	flags.Int("processing-buffer-size", 0, "Number of blocks to buffer after undo before processing (0 = disabled)")
	flags.Int64("processing-buffer-bytes", 0, "Total byte budget for processing buffer (0 = disabled)")
	flags.Int("processing-buffer-max-items", 4096, "Max items allowed in processing buffer (fuse)")

	// (Deprecated token-related flags removed)

	// Debug flags
	flags.Bool("debug", false, "Enable debug logging (shows detailed processing information)")

	// (Deprecated hex-encode-bytes flag removed)
...
func sinkRunE(cmd *cobra.Command, args []string) error {
	...
	schemaFastPath := sflags.MustGetBool(cmd, "schema-fast-path")

	// Get explosion configuration from flags
	explodeField := sflags.MustGetString(cmd, "explode-field")
	explodeWorkers := sflags.MustGetInt(cmd, "explode-workers")
	explodeAlwaysParallel := sflags.MustGetBool(cmd, "explode-always-parallel")

	// Processing buffer configuration (post-undo)
	...
	// (Deprecated omit-empty-strings removed)

	// Get debug configuration
	debugMode := sflags.MustGetBool(cmd, "debug")

	// (Deprecated hex-encode-bytes removed)
	...
	// (Deprecated token GRPC configuration validation removed)
	...
	// (Deprecated token normalization logging removed)
	...
	if outputFormat == "schema-registry" {
		logFields = append(logFields,
			...
			zap.Bool("schema_fast_path", schemaFastPath),
		)
	...
	sinkerFactory := kafkaSinker.SinkerFactory(sinker, kafkaSinker.SinkerFactoryOptions{
		...
		SchemaFastPath:        schemaFastPath,
		ExplodeField:          explodeField,
		ExplodeWorkers:        explodeWorkers,
		ExplodeAlwaysParallel: explodeAlwaysParallel,

		// Processing buffer (post-undo)
		ProcessingBufferSize:     processingBufferSize,
		ProcessingBufferBytes:    processingBufferBytes,
		ProcessingBufferMaxItems: processingBufferMaxItems,
