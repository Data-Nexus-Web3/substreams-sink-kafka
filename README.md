...
```bash
go install github.com/streamingfast/substreams-sink-kafka/cmd/substreams-sink-kafka@latest
```
...
# Run with Schema Registry for Snowflake compatibility
substreams-sink-kafka run localhost:9092 path/to/your/substreams.yaml \
  --output-module=map_pools \
  --kafka-topic=blockchain-data \
...
| `--schema-registry-url` | Schema Registry URL (enables Schema Registry integration) | |
| `--schema-subject` | Schema subject name | `{topic}-value` |
| `--schema-auto-register` | Automatically register schemas | `true` |
...
The Kafka sink supports **Confluent Schema Registry** for enterprise-grade schema management and evolution. This solves your Snowflake connector deserialization challenges!

### ✅ Benefits

- **🔧 Snowflake Compatible**: Messages include magic bytes + schema ID + protobuf data
...
- **🚀 Zero Overhead**: Only ~1μs additional latency for schema ID lookup
...
```
[magic_byte][schema_id][protobuf_bytes]
```

The Schema Registry format is **exactly what Snowflake connectors expect**!
...
The sink uses the **Topic Record Name Strategy**:
- Subject format: `{topic}-{record-type}-{key|value}`
- Example: `adr-data-sf.substreams.rpc.v2.BlockScopedData-value`
...
```bash
git clone https://github.com/streamingfast/substreams-sink-kafka
